...
class Board {
    ...
    printBoard() {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            for (let j = 0; j < 8; j++) {
                if (this.board[i][j]) {
                    ...
                    row += '! ';
                    availableMove.push([String.fromCharCode('a'.charCodeAt(0)+i)+(j+1)]);
                } else {
                    row += '▢ ';
                }
            }
        ...
        console.log(`Available Move : ${availableMove}`);
        console.log('-------------------');
    }
...
class Player {
    ...
    playerInput(board) {
        while (true) {
            console.log(`Player ${board.currentPlayer}, enter your move (e.g., d3): `);
            const move = readline.question();
